...
{
    [ParamsSource(nameof(ArgumentsSource))]
    public VersionArgument Version { get; set; } = null!;

    private const string Digits = "0123456789";

    public class VersionArgument(int version, string data)
    {
    ...
    public static IEnumerable<VersionArgument> ArgumentsSource()
    {
        var numericM = ModeErrorLevelVersionCapacityLookup[0][1];
        for (var i = 0; i < 40; i++)
        {
            yield return new(i + 1, new string(Random.Shared.GetItems(Digits.AsSpan(), numericM[i])));
        }
    }
    ...
    public Gma.QrCodeNet.Encoding.QrCode GmaQrCodeNet() => new Gma.QrCodeNet.Encoding.QrEncoder().Encode(Version.Data);

    [Benchmark(Description = "QrCoder")]
    public QRCoder.QRCodeData QrCoder() => QRCoder.QRCodeGenerator.GenerateQrCode(Version.Data, QRCoder.QRCodeGenerator.ECCLevel.Default);

    [Benchmark(Description = "SkiaSharp.QrCode")]
    public SkiaSharp.QrCode.QRCodeData SkiaSharpQrCode() => SkiaGenerator.CreateQrCode(Version.Data, SkiaSharp.QrCode.ECCLevel.M);

    [Benchmark(Description = "ZXing.Net", Baseline = true)]
    public ZXing.QrCode.Internal.QRCode ZXingQrCode() => ZXing.QrCode.Internal.Encoder.encode(Version.Data, ZXing.QrCode.Internal.ErrorCorrectionLevel.M);
